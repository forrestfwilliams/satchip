"""MajorTOM grid definition.

Modified from https://github.com/ESA-PhiLab/Major-TOM/blob/main/MajorTOM/grid.py
Accessed on 3/27/25
"""
...
class Grid:
    """Class for defining a MajorTOM grid."""
    ...
    def get_rows(self) -> tuple:
        ...
        Returns:
            rows: List of strings, each representing a row in the grid.
            latitudes: List of floats, each representing the latitude of a row in the grid.
        """
        # Define set of latitudes to use, based on the grid distance
        arc_pole_to_pole = np.pi * self.RADIUS_EQUATOR
        num_divisions_in_hemisphere = int(np.ceil(arc_pole_to_pole / self.dist))
    ...
    def get_circumference_at_latitude(self, lat: float) -> float:
        """Get the circumference of the cross-section of a sphere at a given latitude.

        Args:
            lat: Latitude in degrees.

        Returns:
            float: Circumference of the cross-section of a sphere at the given latitude.
        """
        radius_at_lat = self.RADIUS_EQUATOR * np.cos(lat * np.pi / 180)
        circumference = 2 * np.pi * radius_at_lat
        return circumference

    def subdivide_circumference(self, lat: float, return_cols: bool = False) -> np.array:
        """Subdivide the circumference of the earth at a given latitude into equal parts as close as possible to dist.

        Args:
            lat: Latitude in degrees.
            return_cols: Whether to return column names.

        Returns:
            np.array: Array of longitudes
        """
        circumference = self.get_circumference_at_latitude(lat)
        num_divisions = np.ceil(circumference / self.dist)
        longitudes = np.linspace(-180, 180, int(num_divisions) + 1)[:-1]
    ...
    def get_points(self) -> tuple:
        ...
        Returns:
            points: GeoDataFrame of grid points.
            points_by_row: List of GeoDataFrames, each containing the points of a row in the grid
        """
        r_idx = 0
        ...
        for r, lat in zip(self.rows, self.lats):
            ...
            for c, lon in zip(cols, lons):
                ...
                center_lon = lon + (1000 * self.dist / 2) / (111_120 * np.cos(center_lat * np.pi / 180))
            ...
            r_idx += 1
        points = gpd.GeoDataFrame(pd.concat(points_by_row), crs='EPSG:4326')
        # points.reset_index(inplace=True,drop=True)
        return points, points_by_row

    def group_points_by_row(self) -> list:
        """Group the points of the grid by row.

        Returns:
            list: List of GeoDataFrames, each containing the points of a row in the grid.
        """
    ...
    def filter_longitude(self, cols: list, lons: list) -> tuple:
        """Filter the longitude values to be within the longitude range.
        ...
        Returns:
            tuple: Filtered column names
        """
        idxs = (lons >= self.longitude_range[0]) * (lons <= self.longitude_range[1])
        cols, lons = cols[idxs], lons[idxs]
        return cols, lons

    def latlon2rowcol(self, lats: list, lons: list, return_idx: bool = False, integer: bool = False) -> list:
        """Convert latitude and longitude to row and column number from the grid.

        Args:
            lats: List of latitudes.
            lons: List of longitudes.
            return_idx: Whether to return the index of the points in the grid.
            integer: Whether to return the row and column numbers as integers.

        Returns:
            list: List containing the row and column numbers.
        """
    ...
    def rowcol2latlon(self, rows: list, cols: list) -> tuple:
        """Convert row and column number to latitude and longitude.
        ...
        Returns:
            tuple: Tuple containing the latitude and longitude.
        """
        point_geoms = [
    ...
    def get_bounded_footprint(self, point: gpd.GeoDataFrame, buffer_ratio: int = 0) -> Polygon:
        """Get the polygon footprint of the grid cell for a given point, bounded by the other grid points' cells.

        Args:
            point: GeoDataFrame containing the point.
            buffer_ratio: Ratio of the grid cell's width/height to buffer by.

        Returns:
            Polygon footprint of the grid cell.
        """
    ...
    def get_majortom_chip_transform(self, point: gpd.GeoDataFrame) -> tuple:
        if self.dist != 10:
        ...
        utm = pyproj.CRS(f'EPSG:{point.epsg}')
        ...
        geotransform = [minx, cell_size, 0, miny + n_cells * cell_size, 0, -cell_size]
        return geotransform

    def get_terramind_chip_transform(self, point: gpd.GeoDataFrame, row: int, col: int) -> tuple:
        if self.dist != 10:
            raise ValueError('This function is only valid for a grid distance of 10 km.')
        ...
        n_cells = 264
        transform = self.get_majortom_chip_transform(point)
        transform[0] += col * n_cells * transform[1]
        transform[3] += row * n_cells * transform[5]
        return transform


def get_utm_zone_from_latlng(latlng: list) -> str:
    """Get the UTM zone from a latlng list and return the corresponding EPSG code.

    Args:
        latlng: The latlng list to get the UTM zone from.
    ...
    zone_number = (np.floor((longitude + 180) / 6)) % 60 + 1
...
def get_product_window(
    lat: float, lon: float, utm_zone: int = 4326, mt_grid_dist: int = 10, box_size: int = 10680
) -> tuple:
    """None.

    Args:
        lat: Latitude of the reference coordinate.
        lon: Longitude of the reference coordinate.
        utm_zone: UTM zone of the reference coordinate.
        mt_grid_dist: Distance of a given Major TOM grid (10 km is the default).
        box_size: Length of the product footprint.

    Returns:
        Polygon footprint of the product, and the UTM CRS.
    """
    ...
    utm_footprint = Polygon([(left, bottom), (right, bottom), (right, top), (left, top)])
